...
function arrayContiene(array, elemento) {
  ...
  // Tu código:
  if (array.find(ele => ele === elemento)) return true;
  else return false;
}
...
function empiezaConNueve(n) {
  ...
  if (cadena.substring(0,1) === "9") return true;
  else return false;
}
...
function todosIguales(arreglo) {
  ...
  for (let i= 1 ; i < arreglo.length ; i++)
    if (valor != arreglo[i]) return false  ;
  return true; 
} 
...
function mesesDelAño(array) {
  ...
  for (let i= 0 ; i < array.length ; i++) {
    for (let j= 0 ; j < mesabuscar.length ; j++)
      if (array[i]==mesabuscar[j]){
        encontrados = encontrados + 1 ;
        newarray.push(mesabuscar[j]);
        break;
    }    
  }
  if (encontrados == mesabuscar.length ) return newarray ;
